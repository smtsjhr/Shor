...
var H_circuit = [1,1,1,1];// new Array(num_qubits).fill(0);
var H_1 = 0;
var H_2 = 0;
var H_3 = 0;
var H_4 = 0;
...
function draw() {
  ...
  // slider for time step of circuit
//  stepslider.addEventListener("change", function() {circuit_step = parseInt(stepslider.value);
  //if(circuit_step === 3){measured_shifts.push(Math.floor(Math.random()*period));}
//                     }) 
  ...
  // slider for M, the domain to plot
  //
  //Mslider.addEventListener("change", function() {M = Mslider.value;})
  //ctx.font = '14px Arial';
  //ctx.fillStyle = 'rgba(0,0,0,1)';
  //ctx.fillText('M = ' + M.toString(), canvas.width - 100, canvas.height - 30);
 ...
 // if circuit step is not max step, then increment with time  
  ...
  //console.log('draw_time', circuit_time);
  ...
  // plot points for current state
  //let current_state = states(M,N,a,circuit_step);
  //let num_states = current_state.length;
  //console.log('num_states: ', num_states);
  
  if ((circuit_index === 5 || circuit_index === 6) && circuit_step === 4) {
      //console.log('OG QFT draw');
      //console.log('time', t);
    qft_list = QFTdist(M,period,shift);
    max_prob = Math.max.apply(null,qft_list);
  }
  
  if (circuit_index === 7 || circuit_index === 8 && circuit_step === 3) {
    var list = [];
    for (let s = 0; s < period; s++) {
      var qft = QFTdist(M,period, s);
      list.push(qft); 
    }
    var qft_list = list.flat();
   ...
   for(let x = 0; x < num_states; x++) {
    ...
    //     console.log('q1', qft_list);
    //     console.log('q2', qft_list[x]);
    //    let qft_x = qft_list[x];
    //     console.log('qft_x', qft_x);
    //     console.log('qft_list_x', qft_list[x]);
    //    console.log('failpoint');

       if (qft_list[x]/max_prob > .001) 
       {
            var size_x =   point_size*qft_list[x]/max_prob;
            var grow_x = qft_list[x]/max_prob;
       } 
       else {
            var size_x = 0;
            var grow_x = 0;
  ...
  sample_timer_2 += 1;
  //console.log('timer 2 = ', sample_timer_2)
  ...
  sample_timer_1 += 1;
  //console.log('timer 1 = ', sample_timer_1)
...
function stepup() {
  ...
  if (circuit_step < my_circuit_maxstep[circuit_index]) {
    ...
    //console.log('T: ' , circuit_step);
  
  if (circuit_step === 1) {
      measured_state = false;
      //console.log('T1: ' , circuit_step);
  }
  if (circuit_step === 3) {
    //console.log('T3: ' , circuit_step);
  ...
  if (circuit_step === 4) {
    ...
    if (circuit_index === 5 || circuit_index === 6) {
        //console.log('QFTT: ' , circuit_step);
        
        qft_list = QFTdist(M,period,shift);
        max_prob = Math.max.apply(null,qft_list);
        //console.log('fq1', qft_list);
        //console.log('fq2', qft_list[0]);
    }
    ...
    if (circuit_index === 8) {
      ...
      let measurement_1 = sampler(QFTdist(M, period, random_shift), samples)[1].slice(-1)[0];
      measurements1.push(measurement_1);
      measurement1_counts[measurement_1] += 1;
      measured_state = true;
      measured_state_1 = true;
  ...
  if (circuit_step === 5 && circuit_index === 6) {
    console.log('T5: ' , circuit_step);
    samples.push(Math.random());
    let measurement_1 = sampler(QFTdist(M, period, shift), samples)[1].slice(-1)[0];
    measurements1.push(measurement_1);
    measurement1_counts[measurement_1] += 1;
    measured_state = true;
    measured_state_1 = true;
  ...
  //  console.log('curcuit step = ', circuit_step);
  // console.log('Meas 2 Results');
  //  console.table(measurements2);
  //  console.log('Meas 2 Counts');
  // console.table(measurement2_counts)
  // console.log('Meas 1 Results');
  //  console.table(measurements1);
  // console.log('Meas 1 Counts');
  // console.table(measurement1_counts);
...
function circuit_wireglow(ctx, size, rgba_string, fold, wire_x_pos, wire_y_pos, wire_length, run_time, t) {
  
  let x_pos = wire_x_pos + run_time*(wire_length - wire_x_pos);
  let y_pos = wire_y_pos
  ...
  ctx.fillStyle = 'rgba(30,144,255,0.5)';
   ctx.strokeStyle = 'rgba(0,0,0,0.2)'
 ctx.beginPath();
  ctx.arc(x_pos+5*Math.sin(.3*t)*Math.cos(2*Math.PI*i/fold + t), y_pos+5*Math.sin(.3*t)*Math.sin(2*Math.PI*i/fold + t), size, 0, 2*Math.PI);
  ctx.fill();
   ctx.stroke();
  }
