...
function draw() {
  ...
  if (true) {
    for (let i = 0, S = unique_samples.length; i < S; i++) {
      ...
      let f_sample = f(N, sample ,2);
      if (f_sample === 1 && (sample != 1 && sample != N-1)) {
          text_color = 'Tomato';
      } 
      else {
          text_color = 'Black';
      ...
      single_tick(ctx, 1, 10, 1, f_sample);
      if (sample === new_sample && sample != 0 && sample != N-1) {
        tick_text(ctx, 0, sample, 12, text_color);
        tick_text(ctx, 1, f_sample, 12, text_color);
      }
    }
  }
  ...
  if (showplot_state) {
   ...
   if (showplot_state) {
    for(let x = 0; x < N; x++) {
      if (f(N,x ,2) === 1 && x != 1 && x != N-1) {
        point_color = 'Tomato';
      } 
      else {
        point_color = 'rgba(30,144,255,0.5)';
      } 
    plot_point(ctx, 3, point_color, M, N, x, f(N,x,2));
    } 
  }
  ...
  ctx.fillText('Guesses:  ' + samples.length.toString(), 200, canvas.height -  30);
