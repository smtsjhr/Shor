...
function draw() {
  ...
  ctx.drawImage(state_label, 25 + canvas.width/2 - 0.8*state_label.width/2, 70, 0.8*state_label.width, 0.8*state_label.height);
  ...
  if (step === 0) {
      distribution = periodic_state_dist(M,period,shift);
  }
  if (step === 1) {
    distribution = qft_list;
  }
  ctx.save();
  ctx.translate(x_origin, y_origin + 40);
  ctx.scale(1,-1);
     histogram(ctx, distribution, measurement1_counts, measurements1, 0, sample_animation_time, .5, 0, 0, x_scale, 50, true, 5, 'DodgerBlue', false);
  ctx.restore();
