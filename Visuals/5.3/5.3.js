...
function draw() {
  ...
  ctx.drawImage(function_image, canvas.width/2 - function_image.width/2, 7, 1.1*function_image.width, 1.1*function_image.height);
  ...
  // horizontal axis ticks for 0 and M - 1
  if ( new_sample === 0 ) {
      text_size = 16;
  } 
  else {
  ...
  single_tick(ctx, 0, 10, 1, M-1);
  tick_text(ctx, 0, M-1, text_size, 'Black'); 
  ...
  single_tick(ctx, 1, 10, 1, M-1);
  tick_text(ctx, 1, M-1, text_size, 'Black');
  ...
  for(let i = 0, S = samples.length; i < S; i++) {
    ...
    plot_point(ctx, point_size, point_color, N, N, input, output);
  } 
...
function query() {
  samples.push(Math.floor(M*Math.random()));
